    ...
    )
{
    ...
    if (pProcess == NULL)
    {
        DPRINT( "[MALEBOLGE] %s: No process provided\n", __FUNCTION__ );
    ...
    DPRINT( "[MALEBOLGE] %s: Mapping image '%wZ' with flags 0x%X\n", __FUNCTION__, path, flags );
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        if (!NT_SUCCESS( status ))
            DPRINT( "[MALEBOLGE] %s: Failed to create sync event. Status 0x%X\n", __FUNCTION__, status );
    ...
    if (NT_SUCCESS( status ) && !(flags & KNoExceptions))
    {
        if (dynData.KExecOpt != 0)
        {
            ...
            DPRINT( "[MALEBOLGE] %s: Invalid KEXECUTE_OPTIONS offset\n", __FUNCTION__ );
    }
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        __except (EXCEPTION_EXECUTE_HANDLER) {
            ...
            DPRINT( "[MALEBOLGE] %s: Exception during initialization phase.Exception code 0x%X\n", __FUNCTION__, status );
        }
    }
    ...
    )
{
    ...
    DPRINT( "[MALEBOLGE] %s: Loading image %wZ\n", __FUNCTION__, &pLocalImage->fullPath );
        ...
        if (pManifest && manifestID != 0)
        {
            ...
            if (buffer != NULL)
            {
                ...
                if (NT_SUCCESS( status2 ))
                {
                    ...
                    DPRINT( "[MALEBOLGE] %s: Failed to create temporary manifest. Status code: 0x%X\n", __FUNCTION__, status2 );
    ...
    )
{
    ...
    for (; pImportTbl->Name && NT_SUCCESS( status ); ++pImportTbl)
    {
        ...
        if (!pModule.address && !systemImage)
        {
            ...
            {
                DPRINT( "[MALEBOLGE] %s: Loading missing import %wZ\n", __FUNCTION__, &resolved );
        ...
        if (!pModule.address)
        {
            DPRINT( "[MALEBOLGE] %s: Failed to load import '%wZ'. Status code: 0x%X\n", __FUNCTION__, ustrImpDll, status );
            RtlFreeUnicodeString( &ustrImpDll );
            RtlFreeUnicodeString( &resolved );
    ...
    )
{
    ...
    if (pQueryName == NULL)
    {
        DPRINT( "[MALEBOLGE] %s: Failed to get RtlDosApplyFileIsolationRedirection_Ustr\n", __FUNCTION__ );
        return STATUS_NOT_FOUND;
    }
    ...
    )
{
    ...
    {
        DPRINT( "[MALEBOLGE] %s: Missing parameter\n", __FUNCTION__ );
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (NT_SUCCESS( BBResolveApiSet( pProcess, &filename, baseImage, resolved ) ))
    {
        ...
        DPRINT( "[MALEBOLGE] %s: Resolved image '%wZ' to '%wZ' by ApiSetSchema\n", __FUNCTION__, path, fullResolved );
...
NTSTATUS BBCallInitializers( IN PMMAP_CONTEXT pContext, IN BOOLEAN noTLS )
{
    for (PLIST_ENTRY pListEntry = pContext->modules.Flink; pListEntry != &pContext->modules; pListEntry = pListEntry->Flink)
    {
        ...
        DPRINT( "[MALEBOLGE] %s: Calling '%wZ' initializer\n", __FUNCTION__, pEntry->name );
        ...
        {
            DPRINT( "[MALEBOLGE] %s: Process is terminating, map aborted\n", __FUNCTION__ );
        ...
        if (pEntry->flags & KWipeHeader)
        {
            ...
            else
            {
                ...
                if (NT_SUCCESS( ZwProtectVirtualMemory( ZwCurrentProcess(), &pBase, &hdrSize, PAGE_READWRITE, NULL ) ))
                {
                    ...
                    DPRINT( "[MALEBOLGE] %s: Failed to wipe image header\n", __FUNCTION__ );
            }
        }
...
void BBCallTlsInitializers( IN PMMAP_CONTEXT pContext, IN PVOID imageBase )
{
    ...
    if (!pContext->tlsInitialized && TLS_VAL_T( pHeaders, pTLS, StartAddressOfRawData ) != 0)
    {
        ...
        if (NT_SUCCESS( ZwAllocateVirtualMemory( ZwCurrentProcess(), &pTLSMem, 0, &tlsSize, MEM_COMMIT, PAGE_READWRITE ) ))
        {
            ...
            DPRINT( "[MALEBOLGE] %s: Static TLS buffer: 0x%p\n", __FUNCTION__, pTLSMem );
...
NTSTATUS BBPrepareACTX( IN PUNICODE_STRING pPath, IN BOOLEAN asImage, IN LONG manifestID, IN PMMAP_CONTEXT pContext )
{
    ...
    if (!pCreateActCtxW)
    {
        DPRINT( "[MALEBOLGE] %s: Failed to get CreateActCtxW address\n", __FUNCTION__ );
        status = STATUS_NOT_FOUND;
    }
    ...
    if (!pActivateCtx)
    {
        DPRINT( "[MALEBOLGE] %s: Failed to get RtlActivateActivationContext address\n", __FUNCTION__ );
        status = STATUS_NOT_FOUND;
    }
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        {
            DPRINT( "[MALEBOLGE] %s: CreateActCtxW failed\n", __FUNCTION__ );
    ...
    if (NT_SUCCESS( status ))
    {
        ...
        {
            DPRINT( "[MALEBOLGE] %s: RtlActivateActivationContext failed with code 0x%X\n", __FUNCTION__, pContext->userMem->status );
            status = pContext->userMem->status;
        }
    }
...
NTSTATUS BBCreateExceptionTable64( IN PMMAP_CONTEXT pContext, IN PVOID imageBase )
{
    ...
    if (pExpTable)
    {
        ...
        if (!pAddTable)
        {
            DPRINT( "[MALEBOLGE] %s: Failed to get RtlAddFunctionTable address\n", __FUNCTION__ );
            return STATUS_NOT_FOUND;
        }
        ...
        if (NT_SUCCESS( status ))
        {
            if (pContext->userMem->retVal32 == FALSE)
                DPRINT( "[MALEBOLGE] %s: RtlAddFunctionTable failed\n", __FUNCTION__ );
        }
        else
            DPRINT( "[MALEBOLGE] %s: Failed co call RtlAddFunctionTable. Status code: 0x%X\n", __FUNCTION__, status );
...
NTSTATUS BBLoadLocalImage( IN PUNICODE_STRING path, OUT PVOID* pBase )
{
    ...
    {
        DPRINT( "[MALEBOLGE] %s: No image path or output base\n", __FUNCTION__ );
        return STATUS_INVALID_PARAMETER;
    }
