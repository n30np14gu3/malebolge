...
uintptr_t dereference(uintptr_t address, unsigned int offset) {
	...
	return address + static_cast<int>(offset + *reinterpret_cast<int*>(address + offset) + sizeof(int));
}
...
extern "C" BOOLEAN CleanUnloadedDrivers()
{
	...
	PRTL_PROCESS_MODULES modules = static_cast<PRTL_PROCESS_MODULES>(ExAllocatePoolWithTag(NonPagedPool, bytes, POOLTAG));
	...
	UINT64 mmUnloadedDriversPtr = FindPattern(static_cast<UINT64>(ntoskrnlBase), static_cast<UINT64>(ntoskrnlSize), (BYTE*)"\x4C\x8B\x00\x00\x00\x00\x00\x4C\x8B\xC9\x4D\x85\x00\x74", "xx?????xxxxx?x");
	...
	UINT64 mmUnloadedDrivers = reinterpret_cast<UINT64>(reinterpret_cast<PUCHAR>(mmUnloadedDriversPtr) + *reinterpret_cast<PULONG>(reinterpret_cast
		<PUCHAR>(mmUnloadedDriversPtr) + 3) + 7);
	UINT64 bufferPtr = *reinterpret_cast<UINT64*>(mmUnloadedDrivers);
	...
	*(UINT64*)mmUnloadedDrivers = reinterpret_cast<UINT64>(newBuffer);
...
extern "C" void clean_piddb_cache()
{
	...
	PRTL_PROCESS_MODULES modules = static_cast<PRTL_PROCESS_MODULES>(ExAllocatePoolWithTag(NonPagedPool, bytes, POOLTAG)); // 'ENON'
...
static uintptr_t get_kernel_address(const char* name, size_t& size) {
	...
	pModuleList = static_cast<PRTL_PROCESS_MODULES>(ExAllocatePoolWithTag(NonPagedPool, neededSize, POOLTAG));
...
{
#ifndef DEBUG
	VMProtectBeginMutation("#spoof_drives");
#endif
	INT count = 0;
